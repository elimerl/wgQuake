...
void M_Print(int cx, int cy, const char *str) { M_PrintEx(cx, cy, 8, str); }
...
void M_PrintAlignedEx(int cx, int cy, int align, int dim, qboolean color,
                      const char *str) {
...
void M_PrintScroll(int x, int y, int maxwidth, const char *str, double time,
                   qboolean color) {
...
static void M_PrintSubstring(int x, int y, const char *text, int numchars,
                             qboolean color) {
...
static void M_PrintDotFill(int x, int y, const char *text, int cols,
                           qboolean color) {
  char mask = color ? 0x80 : 0;
  while (*text && cols >= 2) {
    M_DrawCharacter(x, y, *text++ ^ mask);
...
int M_PrintWordWrap(int x, int y, const char *text, int width, int height,
                    qboolean color) {
  int maxcols = width / 8;
...
void M_UpdateCursor(int mousey, int starty, int itemheight, int numitems,
                    int *cursor) {
...
void M_UpdateCursorWithTable(int mousey, const int *table, int numitems,
                             int *cursor) {
  int i, dy;
...
void M_List_DrawSearch(const menulist_t *list, int cx, int cy, int maxlen) {
  ...
  M_DrawCharacter(cx + (i - ofs) * 8, cy,
                  list->search.errtimeout ? 11 ^ 128 : 11);
  GL_SetCanvasColor(1.f, 1.f, 1.f, 1.f);
}
...
qboolean M_List_SelectNextMatch(menulist_t *list, qboolean (*match_fn)(int idx),
                                int start, int dir, qboolean wrap) {
  int i, j;
...
qboolean M_List_SelectNextActive(menulist_t *list, int start, int dir,
                                 qboolean wrap) {
  ...
  case K_KP_PGDN:
    ...
    if (list->search.len) {
      ...
      if (list->cursor - list->scroll < list->viewsize - 1)
        sel = M_List_SelectNextActive(list, list->scroll + list->viewsize - 1,
                                      1, false);
      else
        sel = M_List_SelectNextActive(list, list->cursor + list->viewsize - 1,
                                      1, false);
      if (!sel)
        M_List_SelectNextActive(list, list->numitems - 1, -1, false);
    }
...
void M_Maps_Draw(void) {
  ...
  for (i = 0; i < numvis; i++) {
    ...
    if (!item->source) {
      M_PrintWhite(x + (cols - strlen(item->name)) / 2 * 8, y + i * 8,
                   item->name);
    } else {
      char buf[256];
      if (mapsmenu.list.search.len > 0)
        COM_TintSubstring(item->name, mapsmenu.list.search.text, buf,
                          sizeof(buf));
      else
      ...
      if (!message || message[0]) {
        if (!message) {
          ...
          COM_TintSubstring(message, mapsmenu.list.search.text, buf,
                            sizeof(buf));
        else
  ...
  str = va("%d-%d of %d", firstvismap + 1, firstvismap + numvismaps,
           mapsmenu.mapcount);
  ...
  M_List_DrawSearch(&mapsmenu.list, x, y + mapsmenu.list.viewsize * 8 + 4,
                    namecols);
}

void M_Maps_Char(int key) { M_List_Char(&mapsmenu.list, key); }
...
void M_SetSkillMenuMap(const char *name) {
  ...
  if (!Mod_LoadMapDescription(m_skill_maptitle, sizeof(m_skill_maptitle),
                              name) ||
      !m_skill_maptitle[0])
...
void M_Menu_Skill_f(void) {
  ...
  q_snprintf(autosave, sizeof(autosave), "%s/autosave/%s.sav", com_gamedir,
             m_skill_mapname);
...
void M_Skill_Draw(void) {
  ...
  M_PrintScroll(x, 32, 30 * 8, m_skill_maptitle, m_skill_ticker.scroll_time,
                false);
  ...
  if (m_skill_canresume) {
    ...
    if (m_skill_lastplayed) {
      ...
      time(&now);
      COM_DescribeDuration(duration, sizeof(duration),
                           difftime(m_skill_lastplayed, now));
...
void M_Setup_Mousemove(float cx, float cy) {
  ...
  M_UpdateCursorWithTable(cy, setup_cursor_table, NUM_SETUP_CMDS,
                          &setup_cursor);
...
================
*/
void M_Menu_Video_f(void) { M_Options_Init(m_video); }
...
================
*/
void M_Menu_Gamepad_f(void) { M_Options_Init(m_gamepad); }
...
#define OPTIONS_LIST(begin_menu, item, end_menu)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
  begin_menu(OPTIONS, m_options, "") item(OPT_PREVIEW, "Live Preview") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
      OPT_GAMMA,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
      "Brightness") item(OPT_CONTRAST,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
                         "Contrast") item(OPT_VIDEO,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
                                          "Display") item(OPT_GRAPHICS,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
                                                          "Graphics")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            \
      item(OPT_INTERFACE, "Interface") item(SPACER, "") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
          OPT_CUSTOMIZE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
          "Key Setup") item(OPT_GAMEPAD,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
                            "Gamepad") item(OPT_MOUSESPEED,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
                                            "Mouse Speed") item(OPT_INVMOUSE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                                "Invert "                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
                                                                "Mouse")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
          item(OPT_SNDVOL, "Sound Volume") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
              OPT_MUSICVOL,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
              "Music Volume") item(OPT_GAME,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
                                   "Game") item(OPT_MODS,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
                                                "Mods") item(SPACER,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
                                                             "") item(OPT_CONSOLE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               \
                                                                      "Consol"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                                      "e")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
              item(OPT_DEFAULTS, "Reset All") end_menu() begin_menu(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
                  VIDEO_OPTIONS, m_video,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
                  TITLE("Display")) item(OPT_RESOLUTION,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
                                         "Resolution") item(OPT_DISPLAYMODE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
                                                            "Display Mode")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
                  item(OPT_REFRESHRATE, "Refresh Rate") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                      OPT_TEST_VIDEO, "Test changes") item(OPT_APPLY_VIDEO,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
                                                           "Apply changes")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
                      item(SPACER, "") item(OPT_VSYNC, "Vertical Sync") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                          OPT_FPSLIMIT,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
                          "FPS Limit") end_menu() begin_menu(GRAPHICS_OPTIONS,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                             m_graphics,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
                                                             TITLE(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
                                                                 "Graphics"))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                          item(OPT_SOFTEMU, "8-bit Mode") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
                              OPT_SOFTEMU_MDL,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                              "Model Warping") item(OPT_MD5,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
                                                    "Models") item(OPT_ANIMLERP,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
                                                                   "Animation"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                                   "s")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
                              item(OPT_TEXFILTER, "Textures") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
                                  OPT_ANISO,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
                                  "Anisotropic") item(SPACER,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                      "") item(OPT_RENDERSCALE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
                                                               "Render Scale")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                  item(OPT_FSAA, "Antialiasing") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
                                      OPT_FSAA_MODE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
                                      "AA Mode") item(SPACER,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                      "") item(OPT_ALPHAMODE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                               "Transparency")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                      item(OPT_PARTICLES, "Particles") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
                                          OPT_WATERWARP,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
                                          "Underwater FX") item(OPT_DLIGHTS,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
                                                                "Dynamic "                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
                                                                "Lights")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
                                          end_menu() begin_menu(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
                                              INTERFACE_OPTIONS, m_interface,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                              TITLE("Interface"))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
                                              item(OPT_UISCALE, "Scale") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                  OPT_PIXELASPECT,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               \
                                                  "Pixels") item(OPT_UIMOUSE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                                 "Mouse")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
                                                  item(SPACER,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                       "") item(OPT_HUDSTYLE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                                "HUD Layout")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                      item(OPT_SBALPHA, "HUD "                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                                        "Alph"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                                        "a") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               \
                                                          OPT_HUDLEVEL,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
                                                          "HUD Detail")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
                                                              ...
                                                              item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
                                                                  ...
                                                                  item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
                                                                      ...
                                                                      item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
                                                                          ...
                                                                          "P"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                                          "S") item(SPACER,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
                                                                                    "") item(OPT_MENUBGSTYLE, "Background") item(OPT_MENUBGALPHA, "BG Alpha") item(OPT_CENTERPRINTBG, "Center Text BG") item(OPT_CONALPHA, "Console Alpha") item(OPT_CONBRIGHTNESS, "Darken Console") item(OPT_CONFIRMQUIT, "Quit Prompt") item(OPT_STARTDEMOS,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
                                                                                                                                                                                                                                                                                                                                "Start Demos") end_menu() begin_menu(GAMEPAD_OPTIONS, m_gamepad, TITLE("Gamepad")) item(GPAD_OPT_DEVICE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
                                                                                                                                                                                                                                                                                                                                                                                                                        "Device") item(SPACER,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                                                                                                                                                                                                                                                                                                                                                                                                       "") item(SPACER, "") item(GPAD_OPT_SENSX,                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                 "Yaw Speed") item(GPAD_OPT_SENSY,                                                                                                                                                                                                                                                                                                                                                                                                                                               \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   "Pitch Speed") item(GPAD_OPT_INVERT,                                                                                                                                                                                                                                                                                                                                                                                                                          \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       "Invert Pitch") item(GPAD_OPT_SWAP_MOVELOOK, "Look Stick") item(SPACER, "") item(GPAD_OPT_EXPONENT_LOOK, "Look Accel") item(GPAD_OPT_EXPONENT_MOVE,                                                                                                                                                                                                                                                                                       \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   "Move Accel") item(SPACER, "") item(GPAD_OPT_DEADZONE_LOOK,                                                                                                                                                                                                                                                   \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       "Look Deadzone") item(GPAD_OPT_DEADZONE_MOVE, "Move Deadzone") item(GPAD_OPT_DEADZONE_TRIG,                                                                                                                                                                               \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           "Trigger Thresh") item(SPACER, "") item(GPAD_OPT_RUMBLE, "Vibration") item(SPACER,                                                                                                                    \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      "") item(GPAD_OPT_GYROENABLE,                                                                                              \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               "Gyro") item(GPAD_OPT_FLICKSTICK,                                                                                 \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            "Flick Stick") item(GPAD_OPT_GYROMODE,                                                               \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "Gyro Button") item(GPAD_OPT_GYROAXIS,                                           \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    "Gyro Axis") item(GPAD_OPT_GYROSENSX,                        \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      "Gyro Yaw Speed") item(GPAD_OPT_GYROSENSY, \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             "Gyro Pitch Speed") \
                                                                          item(GPAD_OPT_GYRONOISE, "Gyro Noise Thresh") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                                              GPAD_OPT_CALIBRATE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
                                                                              "Calibrate")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
                                                                              end_menu() begin_menu(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
                                                                                  GAME_OPTIONS,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
                                                                                  ...
                                                                                  TITLE(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
                                                                                      "Game")) item(OPT_FOV,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
                                                                                                    "Field Of View")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
                                                                                  item(OPT_FOVDISTORT, "Gun Distortion") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
                                                                                      OPT_RECOIL,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
                                                                                      "Recoil") item(OPT_VIEWBOB,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
                                                                                                     "View Bob") item(OPT_ANGLELIMITS,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
                                                                                                                      "Angle Limits")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            \
                                                                                      item(OPT_ALWAYSRUN, "Always Run") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                                                          SPACER,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
                                                                                          "") item(OPT_ALWAYSMLOOK, "Mouse Look")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
                                                                                          item(OPT_LOOKSTRAFE, "Look Strafe") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
                                                                                              OPT_LOOKSPRING,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
                                                                                              "Look Spring")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
                                                                                              item(SPACER, "") item(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
                                                                                                  OPT_LANGUAGE,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
...
#define BEGIN_MENU_OPT(prefix, state, desc)                                    \
  prefix##_BEGIN, _##prefix##_REWIND = prefix##_BEGIN - 1,
...
// Add NUMITEMS values
#define ADD_NUMITEMS(prefix, state, desc)                                      \
  prefix##_NUMITEMS = prefix##_END - prefix##_BEGIN,
...
#define ADD_SUBMENU(prefix, state, desc)                                       \
  {desc, state, prefix##_BEGIN, prefix##_NUMITEMS},
...
static qboolean M_Options_IsEnabled(int index) {
  ...
  if (index > GAMEPAD_OPTIONS_BEGIN && index < GAMEPAD_OPTIONS_END &&
      !IN_HasGamepad())
    return false;
...
static qboolean M_Options_WantsConsole(void) {
  return optionsmenu.preview.id == OPT_CONALPHA ||
         optionsmenu.preview.id == OPT_CONBRIGHTNESS;
}
...
static float M_Options_PreviewAlpha(void) { return optionsmenu.preview.frac; }
...
static void M_Options_Preview(int id) {
  if (cls.state == ca_connected && cls.signon == SIGNONS) {
    if (id < GRAPHICS_OPTIONS_BEGIN || id >= GRAPHICS_OPTIONS_END) {
      ...
      case OPT_CENTERPRINTBG:
        ...
        SCR_CenterPrint("Certain messages appear inconveniently\n"
  ...
  optionsmenu.preview.hold_time =
      (id == OPT_WATERWARP) ? PREVIEW_LONG_HOLD_TIME : PREVIEW_HOLD_TIME;
}
...
void M_Options_Init(enum m_state_e state) {
  ...
  if (!M_Options_IsSelectable(optionsmenu.list.cursor))
    M_List_SelectNextActive(&optionsmenu.list, optionsmenu.list.cursor, 1,
                            true);

  M_Options_UpdateLayout();
}

void M_Menu_Options_f(void) { M_Options_Init(m_options); }
...
void M_AdjustSliders(int dir) {
  ...
  case OPT_UIMOUSE: // UI mouse support
    M_Options_SetUIMouse((M_Options_GetUIMouse() + UI_MOUSE_NUMSETTINGS + dir) %
                         UI_MOUSE_NUMSETTINGS);
    break;
  ...
  case OPT_HUDSTYLE: // hud style
    Cvar_SetValueQuick(&scr_hudstyle, ((int)q_max(scr_hudstyle.value, 0.f) +
                                       (int)HUD_COUNT + dir) %
                                          (int)HUD_COUNT);
    break;
  ...
  case OPT_LANGUAGE:
    Cbuf_AddText(
        va("cycle%s language auto english french german italian spanish\n",
           dir > 0 ? "" : "back"));
    break;
  ...
  case OPT_ANISO:
    ...
    break;
  case OPT_TEXFILTER:
    Cbuf_AddText("cycle gl_texturemode GL_NEAREST_MIPMAP_LINEAR "
  ...
  case OPT_MENUBGALPHA:
    Cvar_SetValueQuick(&scr_menubgalpha,
                       CLAMP(0.0f, scr_menubgalpha.value - dir * 0.1f, 1.f));
    break;
  case OPT_CONALPHA:
    Cvar_SetValueQuick(&scr_conalpha,
                       CLAMP(0.0f, scr_conalpha.value - dir * 0.1f, 1.f));
    break;
  case OPT_CONBRIGHTNESS:
    Cvar_SetValueQuick(&scr_conbrightness,
                       CLAMP(0.0f, scr_conbrightness.value - dir * 0.1f, 1.f));
    break;
  case OPT_CENTERPRINTBG:
  ...
  case GPAD_OPT_DEVICE:
    ...
    // the Steam Deck.
    IN_UseNextGamepad(dir, m_lastkey < (int)K_GAMEPAD_BEGIN ||
  ...
  case GPAD_OPT_EXPONENT_LOOK:
    Cvar_SetValueQuick(&joy_exponent,
                       CLAMP(MIN_JOY_EXPONENT, joy_exponent.value + dir * 0.5f,
                             MAX_JOY_EXPONENT));
    break;
  ...
  case GPAD_OPT_GYROMODE:
    Cvar_SetValueQuick(
        &gyro_mode, (int)(q_max(gyro_mode.value, 0.f) + GYRO_MODE_COUNT + dir) %
                        GYRO_MODE_COUNT);
    break;
...
void M_DrawSliderWithMarkers(int x, int y, float range,
                             const slidermarker_t *markers, int nummarkers,
                             const char *desc) {
  int i;
  ...
  for (i = 0; i < nummarkers; i++) {
    ...
    Draw_CharacterEx(x + (SLIDER_RANGE - 1) * 8 * CLAMP(0.f, marker->frac, 1.f),
                     y + marker->yofs, CHARSIZE, CHARSIZE, marker->glyph);
  }

  Draw_CharacterEx(x + (SLIDER_RANGE - 1) * 8 * range, y, CHARSIZE, CHARSIZE,
                   131);
...
void M_DrawThresholdSlider(int x, int y, float range, qboolean enabled,
                           float live, const char *desc) {
...
qboolean M_SetSliderValue(int option, float f) {
  ...
  case OPT_ANISO:
    ...
    Cvar_SetValueQuick(&gl_texture_anisotropy,
                       CLAMP(1, (int)f, (int)gl_max_anisotropy));
    return true;
  case OPT_FSAA:
...
static void M_Options_DrawItem(int y, int item) {
  ...
  COM_TintSubstring(options_names[item], optionsmenu.list.search.text, buf,
                    sizeof(buf));
  ...
  case OPT_FSAA:
    ...
    M_DrawSlider(x, y, r,
                 framebufs.scene.samples >= 2
                     ? va("%ix", framebufs.scene.samples)
                     : "Off");
    break;
  case OPT_FSAA_MODE:
  ...
  case OPT_RENDERSCALE:
    ...
    M_DrawSlider(x, y, r,
                 r_refdef.scale >= 2 ? va("1/%i", r_refdef.scale) : "Off");
    break;
  case OPT_ANISO:
    r = GetLogFraction(gl_texfilter.anisotropy, 1.f, gl_max_anisotropy);
    M_DrawSlider(x, y, r,
                 gl_texfilter.anisotropy >= 2.f
                     ? va("%ix", (int)gl_texfilter.anisotropy)
                     : "Off");
    break;
  case OPT_TEXFILTER:
  ...
  case GPAD_OPT_DEVICE:
    ...
    if (!str)
      str = "None";
    M_PrintWordWrap(x, y, str, 320 - x, 16,
  ...
  case GPAD_OPT_DEADZONE_LOOK:
    ...
    l = (IN_GetRawLookMagnitude() - MIN_STICK_DEADZONE) /
        (MAX_STICK_DEADZONE - MIN_STICK_DEADZONE);
    M_DrawThresholdSlider(x, y, r, selected && IN_HasGamepad(), l,
  ...
  case GPAD_OPT_GYRONOISE:
    ...
    l = (IN_GetRawGyroMagnitude() - MIN_GYRO_NOISE_THRESH) /
        (MAX_GYRO_NOISE_THRESH - MIN_GYRO_NOISE_THRESH);
    M_DrawThresholdSlider(x, y, r,
                          selected && IN_HasGyro() && gyro_enable.value, l,
...
static void M_Options_UpdatePreview(void) {
  if (optionsmenu.preview.frac != optionsmenu.preview.frac_target) {
    if (optionsmenu.preview.frac < optionsmenu.preview.frac_target) {
      ...
      optionsmenu.preview.frac =
          q_min(optionsmenu.preview.frac, optionsmenu.preview.frac_target);
    } else {
      optionsmenu.preview.frac -= host_rawframetime / PREVIEW_FADEIN_TIME;
      optionsmenu.preview.frac =
          q_max(optionsmenu.preview.frac, optionsmenu.preview.frac_target);
      if (optionsmenu.preview.frac == optionsmenu.preview.frac_target) {
        if (M_Options_ForcedCenterPrint())
          SCR_CenterPrint("");
        optionsmenu.preview.id = -1;
...
static void M_Options_DrawFadeScreen(void) {
  ...
  if ((unsigned int)(optionsmenu.preview.id - firstvis) <
      (unsigned int)numvis) {
...
void M_Options_Draw(void) {
  ...
  if (optionsmenu.subtitle && optionsmenu.subtitle[0])
    M_PrintWhite((320 - 8 * strlen(optionsmenu.subtitle)) / 2, y + 32,
...
void M_Options_Char(int key) { M_List_Char(&optionsmenu.list, key); }
...
static void M_Keys_Populate(void) {
  ...
  for (i = 0; i < NUMCOMMANDS; i++) {
    ...
    if (!hipnotic && (strcmp(menubinds[i].command, "impulse 225") == 0 ||
                      strcmp(menubinds[i].command, "impulse 226") == 0))
      continue;
...
void M_Keys_Draw(void) {
  ...
  // draw tabs
  M_PrintAlignedEx(320 * 1 / 4, y + 32, ALIGN_CENTER, 8,
                   keysmenu.devicemask == KDM_GAMEPAD, "Keyboard & Mouse");
  M_PrintAlignedEx(320 * 3 / 4, y + 32, ALIGN_CENTER, 8,
                   keysmenu.devicemask != KDM_GAMEPAD, "Gamepad");
  ...
  while (numvis-- > 0) {
    ...
    if (keysmenu.items[i].command[0]) {
      ...
      COM_TintSubstring(keysmenu.items[i].description,
                        keysmenu.list.search.text, buf, sizeof(buf));
  ...
  if (bind_grab)
    ...
    hint =
        IN_GetLastActiveDeviceType() == KD_GAMEPAD
            ? va("%s = change, %s = clear",
                 Key_KeynumToFriendlyString(K_ABUTTON),
                 Key_KeynumToFriendlyString(K_YBUTTON))
            : va("%s = change, %s = clear", Key_KeynumToFriendlyString(K_ENTER),
...
void M_Keys_Char(int key) { M_List_Char(&keysmenu.list, key); }
...
textmode_t M_Quit_TextEntry(void) { return TEXTMODE_NOPOPUP; }
...
void M_LanConfig_Mousemove(float cx, float cy) {
  ...
  M_UpdateCursorWithTable(cy, lanConfig_cursor_table,
                          NUM_LANCONFIG_CMDS - StartingGame, &lanConfig_cursor);
...
void M_GameOptions_Mousemove(float cx, float cy) {
  ...
  M_UpdateCursorWithTable(cy, gameoptions_cursor_table, NUM_GAMEOPTIONS,
                          &gameoptions_cursor);
...
void M_Mods_Draw(void) {
  ...
  for (i = 0; i < numvis; i++) {
    ...
    if (!item->source) {
      ...
      if (modsmenu.list.search.len > 0)
        COM_TintSubstring(item->name, modsmenu.list.search.text, tinted,
                          sizeof(tinted));
      else
      ...
      if (Modlist_GetStatus(item->source) == MODSTATUS_INSTALLING) {
        ...
        q_snprintf(buf, sizeof(buf), "\20%3.0f%%\21 %s", 100.0 * progress,
                   item->name);
        if (flash)
          mask ^= 128;
      } else
      ...
      if (message && message[0]) {
        if (modsmenu.list.search.len > 0)
          COM_TintSubstring(message, modsmenu.list.search.text, buf,
                            sizeof(buf));
        else
          q_strlcpy(buf, message, sizeof(buf));
  ...
  str = va("%d-%d of %d", firstvismod + 1, firstvismod + numvismods,
           modsmenu.modcount);
  M_Print(x + (cols - strlen(str)) * 8, y - 24, str);
...
void M_Mods_Char(int key) { M_List_Char(&modsmenu.list, key); }
...
qboolean M_WantsConsole(void) {
  return key_dest == key_menu && M_GetBaseState(m_state) == m_options &&
         M_Options_WantsConsole();
}

qboolean M_ForcedCenterPrint(float *alpha) {
  qboolean forced =
      (key_dest == key_menu && M_GetBaseState(m_state) == m_options)
          ? M_Options_ForcedCenterPrint()
          : false;
  if (alpha)
...
static qboolean M_CheckCustomGfx(const char *custompath, const char *basepath,
                                 int knownlength, const unsigned int *hashes,
                                 int numhashes) {
...
void M_CheckMods(void) {
  const unsigned int main_hashes[] = {0x136bc7fd, 0x90555cb4},
                     sp_hashes[] = {0x86a6f086}, sgl_hashes[] = {0x7bba813d};

  m_main_mods = M_CheckCustomGfx("gfx/menumods.lmp", "gfx/mainmenu.lmp", 26888,
                                 main_hashes, countof(main_hashes));

  m_singleplayer_showlevels =
      M_CheckCustomGfx("gfx/sp_maps.lmp", "gfx/sp_menu.lmp", 14856, sp_hashes,
                       countof(sp_hashes));

  m_skill_usegfx = M_CheckCustomGfx("gfx/skillmenu.lmp", "gfx/sp_menu.lmp",
                                    14856, sp_hashes, countof(sp_hashes));

  m_skill_usecustomtitle =
      M_CheckCustomGfx("gfx/p_skill.lmp", "gfx/ttl_sgl.lmp", 6728, sgl_hashes,
                       countof(sgl_hashes));
}
