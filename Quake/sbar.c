...
void Sbar_DrawPicAlpha(int x, int y, qpic_t *pic, float alpha) {
  ...
  Draw_Pic(x, y + 24, pic);
  GL_SetCanvasColor(1, 1, 1,
                    1); // ericw -- changed from glColor3f to work around intel
...
void Sbar_DrawInventory(void) {
  ...
  Sbar_DrawPicAlpha(0, -24, Sbar_InventoryBarPic(),
  ...
  if (scr_viewsize.value < 110) {
        ...
        if (cl.items & (1 << hipweapons[i])) {
          ...
          if (i == 2) {
            if (cl.items & HIT_PROXIMITY_GUN) {
              if (flashon) {
                grenadeflashing = 1;
                Sbar_DrawPic(24, -69 - scoreboard_y_gap - (16 * 9) + 4 * 16,
            ...
            Sbar_DrawPic(24, -69 - scoreboard_y_gap - (16 * 9) + (7 + i) * 16,
    ...
    if (rogue) {
      ...
      if (cl.stats[STAT_ACTIVEWEAPON] >= RIT_LAVA_NAILGUN) {
        for (i = 0; i < 5; i++) {
          if (cl.stats[STAT_ACTIVEWEAPON] == (RIT_LAVA_NAILGUN << i)) {
            Sbar_DrawPic(24, -69 - scoreboard_y_gap - (16 * 7) + (i + 2) * 16,
                         rsb_weapons[i]);
          }
        }
      }
    ...
    {
      for (i = 0; i < 4; i++) {
        Draw_SubPic(4, -45 - scoreboard_y_gap + (i * 11), 44, 11, pic,
                    1 / 320.f + i * (48 / 320.f), 0.f, 44 / 320.f, 11 / 24.f,
                    NULL, scr_sbaralpha.value);
      }
    } else {
      for (i = 0; i < 4; i++) {
        Draw_SubPic(6, -45 - scoreboard_y_gap + (i * 11), 42, 11, pic,
                    3 / 320.f + i * (48 / 320.f), 0.f, 42 / 320.f, 11 / 24.f,
                    NULL, scr_sbaralpha.value);
      }
    }
    ...
    // ammo counts
    for (i = 0; i < 4; i++) {
      Sbar_DrawSmallNum(13, -69 - scoreboard_y_gap + (i * 11),
    ...
    if (!rogue) {
      ...
      if (has_a_sigil)
        Draw_SubPic(16, -16 - scoreboard_y_gap + 24, 32, 16, sb_ibar,
                    1.f - 32 / 320.f, 8 / 24.f, 32 / 320.f, 16 / 24.f, NULL,
                    1.f);
        ...
        if (cl.items & (1 << hipweapons[i])) {
          ...
          if (i == 2) {
            if (cl.items & HIT_PROXIMITY_GUN) {
              if (flashon) {
                grenadeflashing = 1;
                Sbar_DrawPic(x - (active ? 24 : 18), y - ROW_HEIGHT * 4,
                             hsb_weapons[flashon][2]);
              }
            }
          } else if (i == 3) {
            ...
            Sbar_DrawPic(x - (active ? 24 : 18), y - ROW_HEIGHT * (i + 7),
                         hsb_weapons[flashon][i]);
        ...
        hudstyle == HUD_QUAKEWORLD) // right side, 2x2
    {
      ...
      for (i = 0; i < 2; i++)
        Draw_SubPic(x, y + 24 - 10 * i, ITEM_WIDTH * 2, 10, pic,
                    i * (2 * 48 / 320.f), 0.f, 2 * 48 / 320.f, 10 / 24.f, NULL,
                    scr_sbaralpha.value);

      for (i = 0; i < 4; i++)
        Sbar_DrawSmallNum(x + 11 + ITEM_WIDTH * (i & 1), y - 10 * (i >> 1),
                          cl.stats[STAT_SHELLS + i]);
    } else // bottom center, 4x1
    {
      x = (int)((glcanvas.right + glcanvas.left) * 0.5f + 0.5f) - 96;
      y = (int)(glcanvas.bottom - 9 + 0.5f);
      Draw_SubPic(x, y, 192, 10, pic, 0.f, 0.f, 192 / 320.f, 10 / 24.f, NULL,
                  scr_sbaralpha.value);
      for (i = 0; i < 4; i++)
...
static void Sbar_DrawSigils(void) {
  ...
  if (sb_showscores)
    ...
    y = -4;
  Draw_SubPic(x, y + 24, 32, 16, sb_ibar, 1.f - 32 / 320.f, 8 / 24.f,
              32 / 320.f, 16 / 24.f, NULL, 1.f);
...
void Sbar_Draw(void) {
  ...
  if (hudstyle == HUD_CLASSIC || hudstyle == HUD_QUAKEWORLD) {
               ...
               120) // johnfitz -- check viewsize instead of sb_lines
    {
      ...
      Sbar_DrawNum(x + 32, y, cl.stats[STAT_HEALTH], 3,
                   cl.stats[STAT_HEALTH] <= 25);
      ...
      Sbar_DrawNum(x - 48, y, cl.stats[STAT_AMMO], 3,
