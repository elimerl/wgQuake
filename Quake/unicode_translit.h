...
print('/%s Single-character transliterations (based on Unidecode) %s/' % ('*',
'*')) # shenanigans to keep the Python code C comment-friendly with
open(__file__, 'r') as source: print('/%s\n%s\n%s/' % ('*', source.read(), '*'))
print("static const struct { uint16_t code; char remap[2]; }
unicode_translit_src[] =\n{") for codepoint in unicode_ranges: if 0xd800 <=
codepoint <= 0xdfff: continue srcstr = chr(codepoint) src = srcstr.encode()
    translit = unidecode(srcstr).encode()
    if src != translit and 0 < len(translit) <= min(2, len(src)):
        print('\t{0x%04X, {%s}}, // %s' % (codepoint, ', '.join([char_literal(c)
for c in translit]), unicodedata.name(srcstr))) print("};")
*/
static const struct {
  uint16_t code;
  char remap[2];
    ...
    {0x01C5,
     {'D', 'z'}}, // LATIN CAPITAL LETTER D WITH SMALL LETTER Z WITH CARON
