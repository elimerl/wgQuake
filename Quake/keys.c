...
static const keyname_t keynames[] = {
    ...
    // We don't expose the key names in order to avoid having the player bind
    // them in the console.
    //{"JOY_START", K_START},
    //{"JOY_BACK", K_BACK},
...
void Key_Console(int key) {
  ...
  case K_ABUTTON:
    ...
    key_lines[edit_line][1] = 0; // johnfitz -- otherwise old history items show
                                 // up in the new edit line
    key_linepos = 1;
    key_tabhint[0] = '\0';
...
void Key_EventWithKeycode(int key, qboolean down, int keycode) {
  ...
  // menu on the Steam Deck. Note: shift modifier is currently not supported for
  // emulated char events.
  ...
  if (cls.demoplayback && key_dest == key_game) {
    ...
    case K_CTRL:
      ...
      // state transitions (e.g. pressing esc while still holding left arrow to
      // rewind).
      return;
