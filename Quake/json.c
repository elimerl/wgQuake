...
json_t *JSON_Parse(const char *text) {
  ...
  json = (json_t *)calloc(
      sizeof(json_t) + sizeof(jsonentry_t) * numtokens + len, 1);
  if (!json)
    goto free_tokens;
...
==================
*/
void JSON_Free(json_t *json) { free(json); }
...
==================
*/
const jsonentry_t *JSON_Find(const jsonentry_t *entry, const char *name,
                             jsontype_t type) {
  if (!entry)
    return NULL;
