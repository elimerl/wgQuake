...
==================
*/
static qboolean Chart_Add(chart_t *chart, int w, int h, short *outx,
                          short *outy) {
  int i, x, y;
  if (chart->width < w || chart->height < h)
    Sys_Error("Chart_Add: block too large %dx%d, max is %dx%d", w, h,
...
void GL_BuildLightmaps(void) {
  ...
  Con_DPrintf("Lightmap size:   %d x %d (%d/%d blocks)\n"
              ...
              lightmap_width, lightmap_height, lightmap_count,
              xblocks * yblocks, (lightmap_bytes * lmsize) / (float)0x100000,
  ...
  lightmap_texture = TexMgr_LoadImage(
      cl.worldmodel, "lightmap", lightmap_width, lightmap_height, SRC_LIGHTMAP,
      (byte *)lightmap_data, "", (src_offset_t)lightmap_data,
...
void GL_BuildBModelVertexBuffer(void) {
  ...
  gl_bmodel_vbo = GL_CreateBuffer(GL_ARRAY_BUFFER, GL_STATIC_DRAW, "brushverts",
                                  varray_bytes, varray);
  free(varray);
}
...
void GL_BuildBModelMarkBuffers(void) {
  ...
  if (!idx)
    Sys_Error("GL_BuildBModelMarkBuffers: out of memory (%d indices)",
              numtris * 3);
  leafs =
      (bmodel_gpu_leaf_t *)calloc(cl.worldmodel->numleafs, sizeof(leafs[0]));
  if (!leafs)
  ...
  gl_bmodel_indirect_buffer =
      GL_CreateBuffer(GL_SHADER_STORAGE_BUFFER, GL_DYNAMIC_DRAW,
                      "bmodel indirect cmds", sizeof(cmds[0]) * numtex, cmds);
  gl_bmodel_ibo =
      GL_CreateBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_DYNAMIC_DRAW,
                      "bmodel indices", sizeof(idx[0]) * numtris * 3, idx);
  gl_bmodel_leaf_buffer =
      GL_CreateBuffer(GL_SHADER_STORAGE_BUFFER, GL_STATIC_DRAW, "bmodel leafs",
                      sizeof(leafs[0]) * cl.worldmodel->numleafs, leafs);
  gl_bmodel_surf_buffer =
      GL_CreateBuffer(GL_SHADER_STORAGE_BUFFER, GL_STATIC_DRAW, "bmodel surfs",
                      sizeof(surfs[0]) * cl.worldmodel->numsurfaces, surfs);
  gl_bmodel_marksurf_buffer = GL_CreateBuffer(
      GL_SHADER_STORAGE_BUFFER, GL_STATIC_DRAW, "bmodel marksurfs",
