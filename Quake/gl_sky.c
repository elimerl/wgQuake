...
void Sky_LoadTexture(qmodel_t *mod, texture_t *mt) {
  ...
  q_snprintf(texturename, sizeof(texturename), "%s:%s_back", mod->name,
             mt->name);
  mt->gltexture = TexMgr_LoadImage(mod, texturename, halfwidth, mt->height,
                                   SRC_INDEXED, back_data, "",
                                   (src_offset_t)back_data, TEXPREF_BINDLESS);
  ...
  q_snprintf(texturename, sizeof(texturename), "%s:%s_front", mod->name,
             mt->name);
  mt->fullbright = TexMgr_LoadImage(
      mod, texturename, halfwidth, mt->height, SRC_INDEXED, front_data, "",
      (src_offset_t)front_data, TEXPREF_ALPHA | TEXPREF_BINDLESS);
...
void Sky_LoadTextureQ64(qmodel_t *mod, texture_t *mt) {
  ...
  mt->gltexture =
      TexMgr_LoadImage(mod, texturename, mt->width, halfheight, SRC_INDEXED,
                       back, "", (src_offset_t)back, TEXPREF_BINDLESS);
  ...
  q_snprintf(texturename, sizeof(texturename), "%s:%s_front", mod->name,
             mt->name);
  mt->fullbright = TexMgr_LoadImage(
      mod, texturename, mt->width, halfheight, SRC_RGBA, front_rgba, "",
      (src_offset_t)front_rgba, TEXPREF_ALPHA | TEXPREF_BINDLESS);
...
static void Skywind_Save_f(void) {
  ...
  fprintf(f,
          ...
          "skywind %g %g %g %g\n",
          skybox->wind_dist, skybox->wind_yaw, skybox->wind_period,
          skybox->wind_pitch);
...
static void Skywind_f(void) {
  ...
  if (Cmd_Argc() < 2) {
    Con_Printf("usage:\n"
               ...
               "   \"period\"   is \"%g\"\n"
               "   \"pitch\"    is \"%g\"\n",
               Cmd_Argv(0), skybox->wind_dist, skybox->wind_yaw,
...
void Sky_LoadSkyBox(const char *name) {
      ...
      0) // create a single cubemap texture if all faces are the same size
  {
    ...
    newsky.cubemap = TexMgr_LoadImage(
        cl.worldmodel, filename, samesize, samesize, SRC_RGBA,
  ...
  } else // create a separate texture for each side
  {
    for (i = 0; i < 6; i++) {
      q_snprintf(filename, sizeof(filename), "gfx/env/%s%s", name, suf[i]);
      newsky.textures[i] =
          TexMgr_LoadImage(cl.worldmodel, filename, width[i], height[i],
                           SRC_RGBA, data[i], filename, 0, TEXPREF_NONE);
    }
  }
  Hunk_FreeToLowMark(mark);
...
void Sky_DrawSkyBox(void) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    GL_VertexAttribPointerFunc(0, 3, GL_FLOAT, GL_FALSE, sizeof(verts[0]),
                               ofs + offsetof(struct skyboxvert_s, pos));
    GL_VertexAttribPointerFunc(1, 2, GL_FLOAT, GL_FALSE, sizeof(verts[0]),
