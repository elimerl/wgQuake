...
===============
*/
void CL_AddDemoRewindSound(int entnum, int channel, sfx_t *sfx, vec3_t pos,
                           int vol, float atten) {
  soundevent_t sound;
  ...
  if (cls.demospeed > 0.f) {
    if (cls.signon < SIGNONS) {
      ...
      for (i = 0; i < MAX_LIGHTSTYLES; i++)
        q_strlcpy(demo_rewind.prev.lightstyles[i], cl_lightstyle[i].map,
                  MAX_STYLESTRING);
...
void CL_FinishDemoFrame(void) {
  ...
  if (cls.demospeed > 0.f) // forward playback
  {
    ...
    if (memcmp(&demo_rewind.prev.cshift, &cshift_empty, sizeof(cshift_t)) !=
      ...
      Vec_Append((void **)&demo_rewind.frame_events, 1,
                 &demo_rewind.prev.cshift, sizeof(cshift_t));
    ...
    for (i = 0; i < MAX_LIGHTSTYLES; i++) {
      ...
      Vec_Append((void **)&demo_rewind.frame_events, 1,
                 demo_rewind.prev.lightstyles[i], len);
    ...
    if (lastframe->datasize > 0) {
      ...
      while (begin < end) {
        ...
        case DFE_SOUND: {
          ...
          if (snd.sfx)
            S_StartSound(snd.ent, snd.channel, snd.sfx, snd.pos,
                         snd.volume / 255.0, snd.attenuation / 64.f);
          else
      ...
      if ((double)cl.stats[i] != cl.statsf[i] &&
          (unsigned int)cl.stats[i] <=
              0x00ffffff) { // if the float representation seems to have more
                            // precision then use that, unless its getting huge
                            // in which case we're probably getting fpu
