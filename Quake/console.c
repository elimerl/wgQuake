...
================
*/
static qboolean Con_CanvasToOffset(int x, int y, conofs_t *ofs,
                                   contest_t testmode) {
  qboolean ret = true;
...
================
*/
static qboolean Con_OfsInRange(const conofs_t *ofs, const conofs_t *begin,
                               const conofs_t *end) {
...
================
*/
static qboolean Con_IntersectRanges(conofs_t *begin, conofs_t *end,
...
void Con_Mousemove(int x, int y) {
  if (con_mousestate == CMS_NOTPRESSED) {
    ...
    if (delta < 0) {
      ...
      int scrolled =
          q_min(con_mouseselection.end.line - con_mouseselection.begin.line,
                0) *
          CHARSIZE;
...
static void Con_Dump_f(void) {
  ...
  q_strlcpy(relname, Cmd_Argc() >= 2 ? Cmd_Argv(1) : "condump.txt",
            sizeof(relname));
...
void Con_DrawInput(void) {
  ...
  if (key_tabhint[0]) {
    ...
    for (i = 0;
         ...
         i++)
      Draw_Character((i + 1 + len - ofs) << 3, vid.conheight - 16,
                     key_tabhint[i] | 0x80);
