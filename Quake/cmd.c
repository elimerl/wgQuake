...
static void Cmd_CfgMarker_f(void) { in_cfg_exec = false; }
...
void Cbuf_Waited(void) { cmd_wait = false; }
...
static void Cmd_ListAllContaining(const char *substr) {
  ...
  for (cmd = cmd_functions; cmd; cmd = cmd->next) {
    if (cmd->srctype != src_server && q_strcasestr(cmd->name, substr) &&
      ...
      hits++;
      Con_SafePrintf("   %s\n", COM_TintSubstring(cmd->name, substr, tmpbuf,
                                                  sizeof(tmpbuf)));
    }
  }
...
============
*/
const char *Cmd_Args(void) { return cmd_args; }
...
============
*/
cmd_function_t *Cmd_AddCommand2(const char *cmd_name, xcommand_t function,
