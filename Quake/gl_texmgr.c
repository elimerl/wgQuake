...
cvar_t r_softemu_lightmap_banding = {"r_softemu_lightmap_banding", "-1",
                                     CVAR_ARCHIVE};
cvar_t r_softemu_mdl_warp = {"r_softemu_mdl_warp", "-1", CVAR_ARCHIVE};
cvar_t r_softemu_dither_screen = {"r_softemu_dither_screen", "1.0",
                                  CVAR_ARCHIVE};
cvar_t r_softemu_dither_texture = {"r_softemu_dither_texture", "1.0",
                                   CVAR_ARCHIVE};
...
unsigned int
    d_8to24table_alphabright[256]; // palette with lighting mask in alpha
                                   // channel (0=fullbright, 255=lit)
unsigned int
...
static void TexMgr_CreateSamplers(void) {
  ...
  for (i = 0; i < NUM_GLMODES; i++) {
    GL_SamplerParameteriFunc(gl_samplers[i * 2 + 0], GL_TEXTURE_MAG_FILTER,
                             glmodes[i].magfilter);
    GL_SamplerParameteriFunc(gl_samplers[i * 2 + 0], GL_TEXTURE_MIN_FILTER,
                             glmodes[i].magfilter);
    GL_SamplerParameterfFunc(gl_samplers[i * 2 + 0],
                             GL_TEXTURE_MAX_ANISOTROPY_EXT,
                             gl_texfilter.anisotropy);
    GL_SamplerParameterfFunc(gl_samplers[i * 2 + 0], GL_TEXTURE_LOD_BIAS,
                             gl_texfilter.lodbias);

    GL_SamplerParameteriFunc(gl_samplers[i * 2 + 1], GL_TEXTURE_MAG_FILTER,
                             glmodes[i].magfilter);
    GL_SamplerParameteriFunc(gl_samplers[i * 2 + 1], GL_TEXTURE_MIN_FILTER,
                             glmodes[i].minfilter);
    GL_SamplerParameterfFunc(gl_samplers[i * 2 + 1],
                             GL_TEXTURE_MAX_ANISOTROPY_EXT,
                             gl_texfilter.anisotropy);
    GL_SamplerParameterfFunc(gl_samplers[i * 2 + 1], GL_TEXTURE_LOD_BIAS,
                             gl_texfilter.lodbias);
  }
}
...
static void TexMgr_SetFilterModes(gltexture_t *glt) {
  ...
  if (glt->flags & TEXPREF_NEAREST) {
    ...
    glTexParameterf(glt->target, GL_TEXTURE_MAG_FILTER,
                    glmodes[gl_texfilter.mode].magfilter);
    glTexParameterf(glt->target, GL_TEXTURE_MIN_FILTER,
                    glmodes[gl_texfilter.mode].minfilter);
    glTexParameterf(glt->target, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                    gl_texfilter.anisotropy);
...
void TexMgr_ApplySettings(void) {
  ...
  // if softemu is either 2 & 3 or r_softemu_lightmap_banding is > 0 we override
  // the filtering mode, unless it's GL_NEAREST
  ...
  for (glt = active_gltextures; glt; glt = glt->next) {
    ...
    if (layers > 1)
      Con_SafePrintf("%3i x %4i x %4i %c%c %s\n", layers, glt->width,
                     glt->height, comp, mip, buf);
    else
      Con_SafePrintf("      %4i x %4i %c%c %s\n", glt->width, glt->height, comp,
                     mip, buf);
  ...
  for (glt = active_gltextures; glt; glt = glt->next) {
    ...
    if (!buffer)
      Sys_Error("TexMgr_Imagedump_f: out of memory (%dx%dx%d %d bpp)",
                glt->width, glt->height, glt->depth, channels * 8);

    if (glt->flags & TEXPREF_CUBEMAP) {
      const char *suf[6] = {"ft", "bk", "up", "dn", "rt", "lf"};
      int i;
      for (i = 0; i < 6; i++) {
        q_snprintf(tganame, sizeof(tganame), "imagedump/%s%s.tga", tempname,
                   suf[i]);
        glGetTexImage(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, format,
                      GL_UNSIGNED_BYTE, buffer);
        Image_WriteTGA(tganame, buffer, glt->width, glt->height * glt->depth,
                       channels * 8, true);
      }
    } else {
...
void TexMgr_Init(void) {
  ...
  static byte whitetexture_data[16] = {255, 255, 255, 255, 255, 255,
                                       255, 255, 255, 255, 255, 255,
                                       255, 255, 255, 255}; // white
  static byte greytexture_data[16] = {127, 127, 127, 255, 127, 127,
                                      127, 255, 127, 127, 127, 255,
                                      127, 127, 127, 255}; // 50% grey
  ...
  notexture = TexMgr_LoadImage(NULL, "notexture", 2, 2, SRC_RGBA,
  ...
  nulltexture = TexMgr_LoadImage(
      NULL, "nulltexture", 2, 2, SRC_RGBA, nulltexture_data, "",
      (src_offset_t)nulltexture_data,
      TEXPREF_NEAREST | TEXPREF_PERSIST | TEXPREF_NOPICMIP | TEXPREF_BINDLESS);
  whitetexture = TexMgr_LoadImage(
      NULL, "whitetexture", 2, 2, SRC_RGBA, whitetexture_data, "",
      (src_offset_t)whitetexture_data,
      TEXPREF_NEAREST | TEXPREF_PERSIST | TEXPREF_NOPICMIP | TEXPREF_BINDLESS);
  greytexture = TexMgr_LoadImage(
      NULL, "greytexture", 2, 2, SRC_RGBA, greytexture_data, "",
      (src_offset_t)greytexture_data,
      TEXPREF_NEAREST | TEXPREF_PERSIST | TEXPREF_NOPICMIP | TEXPREF_BINDLESS);
  blacktexture = TexMgr_LoadImage(
      NULL, "blacktexture", 2, 2, SRC_RGBA, blacktexture_data, "",
...
================
*/
static void GL_TexImage(gltexture_t *glt, GLint level, GLint internalformat,
  ...
  case GL_TEXTURE_2D_ARRAY:
    GL_TexImage3DFunc(glt->target, level, internalformat, width, height,
                      glt->depth, 0, format, type, NULL);
    for (i = 0; i < glt->depth; i++)
      GL_TexSubImage3DFunc(glt->target, level, 0, 0, i, width, height, 1,
                           format, type, images ? images[i] : NULL);
    break;

  case GL_TEXTURE_CUBE_MAP:
    for (i = 0; i < 6; i++)
      glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, level, internalformat,
                   width, height, 0, format, type, images ? images[i] : NULL);
    break;

  case GL_TEXTURE_2D:
    glTexImage2D(glt->target, level, internalformat, width, height, 0, format,
                 type, pixels);
    break;
...
static void TexMgr_LoadImage32(gltexture_t *glt, unsigned *data) {
  ...
  GL_Bind(GL_TEXTURE0, glt);
  GL_TexImage(glt, 0, internalformat.id, glt->width, glt->height, GL_RGBA,
  ...
  if (glt->flags & TEXPREF_MIPMAP) {
    if (glt->flags & (TEXPREF_CUBEMAP | TEXPREF_ARRAY)) {
      ...
      for (miplevel = 1; mipwidth > 1 || mipheight > 1; miplevel++) {
        ...
        GL_TexImage(glt, miplevel, internalformat.id, mipwidth, mipheight,
...
static void TexMgr_LoadLightmap(gltexture_t *glt, byte *data) {
  ...
  glt->compression = 1;
  GL_Bind(GL_TEXTURE0, glt);
  GL_TexImage(glt, 0, GL_RGBA8, glt->width, glt->height, gl_lightmap_format,
              GL_UNSIGNED_BYTE, data);
...
================
*/
gltexture_t *TexMgr_LoadImageEx(qmodel_t *owner, const char *name, int width,
                                int height, int depth, enum srcformat format,
                                byte *data, const char *source_file,
...
gltexture_t *TexMgr_LoadImage(qmodel_t *owner, const char *name, int width,
                              ...
                              src_offset_t source_offset, unsigned flags) {
  return TexMgr_LoadImageEx(owner, name, width, height, 1, format, data,
                            source_file, source_offset, flags);
}
...
void TexMgr_ReloadImage(gltexture_t *glt, int shirt, int pants) {
  ...
  if (glt->source_file[0] && glt->source_offset) {
    ...
    data = Image_LoadImage(glt->source_file, (int *)&glt->source_width,
                           (int *)&glt->source_height, &fmt); // simple file
  } else if (!glt->source_file[0] && glt->source_offset) {
  ...
  if (fmt != glt->source_format)
    Sys_Error("Texture %s changed format from %i to %i", glt->name,
              glt->source_format, fmt);
...
void GLPalette_CreateResources(void) {
  ...
  GL_TexImage3DFunc(GL_TEXTURE_3D, 0, GL_R8UI, 128, 128, 128, 0, GL_RED_INTEGER,
...
void GLPalette_UpdateLookupTable(void) {
  ...
  GL_BindImageTextureFunc(0, gl_palette_lut, 0, GL_TRUE, 0, GL_WRITE_ONLY,
                          GL_R8UI);
  GL_BindBufferRange(GL_SHADER_STORAGE_BUFFER, 0, gl_palette_buffer[0], 0,
                     256 * sizeof(GLuint));
  GL_BufferSubDataFunc(GL_SHADER_STORAGE_BUFFER, 0, 256 * sizeof(GLuint),
                       d_8to24table);
...
int GLPalette_Postprocess(void) {
  ...
  GL_BindBufferRange(GL_SHADER_STORAGE_BUFFER, 0, gl_palette_buffer[0], 0,
                     256 * sizeof(GLuint));
  GL_BindBufferRange(GL_SHADER_STORAGE_BUFFER, 1, gl_palette_buffer[1], 0,
                     256 * sizeof(GLuint));
