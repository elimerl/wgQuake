...
static int read_export(fshandle_t *f, const struct upkg_hdr *hdr, int32_t *ofs,
                       int32_t *objsize) {
  char buf[40];
  int idx = 0, t;
...
static int read_typname(fshandle_t *f, const struct upkg_hdr *hdr, int idx,
                        char *out) {
  int i, s;
  long l;
  char buf[64];
...
static int probe_umx(fshandle_t *f, const struct upkg_hdr *hdr, int32_t *ofs,
                     int32_t *objsize) {
  int i, idx, t;
  int32_t s, pos;
  long fsiz;
...
static qboolean S_UMX_CodecInitialize(void) { return true; }
...
static int S_UMX_CodecReadStream(snd_stream_t *stream, int bytes,
                                 void *buffer) {
  return -1;
}
...
static int S_UMX_CodecRewindStream(snd_stream_t *stream) { return -1; }
