...
cvar_t scr_centerprintbg = {
    "scr_centerprintbg", "0",
    CVAR_ARCHIVE}; // 0 = off; 1 = text box; 2 = menu box; 3 = menu strip
...
cvar_t cl_screenshotname = {"cl_screenshotname",
                            "screenshots/%map%_%date%_%time%", CVAR_ARCHIVE};
...
static void SCR_DrawCenterStringBG(int y, float alpha) {
  ...
  if (cl.intermission || q_min(scr_center_lines, scr_center_maxcols) <= 0 ||
      alpha <= 0.f)
    return;
  ...
  case 3:
    Draw_PartialFadeScreen(glcanvas.left, glcanvas.right, y - 4,
                           y + lines * 8 + 4, alpha);
    break;
...
void SCR_ClearCenterString(void) { scr_centertime_off = 0; }
...
====================
*/
static void SCR_ZoomDown_f(void) { cl.zoomdir = 1.f; }
...
====================
*/
static void SCR_ZoomUp_f(void) { cl.zoomdir = -1.f; }
...
void SCR_DrawDevStats(void) {
  ...
  sprintf(str, "Packet   |%5i %5i", dev_stats.packetsize,
          dev_peakstats.packetsize);
  Draw_String(x, (y++) * 8 - x, str);

  sprintf(str, "Visedicts|%5i %5i", dev_stats.visedicts,
  ...
  sprintf(str, "GL upload|%4iK %4iK", dev_stats.gpu_upload / 1024,
...
static void SCR_SetupProjToCanvasMap(canvasmap_t *remap) {
  ...
  height = r_refdef.vrect.height / (float)glheight *
           (glcanvas.top - glcanvas.bottom);
  xmin = glcanvas.left +
         (r_refdef.vrect.x / (float)glwidth) * (glcanvas.right - glcanvas.left);
  ymin =
      glcanvas.bottom - ((glheight - r_refdef.vrect.y - r_refdef.vrect.height) /
                         (float)glheight) *
...
==============
*/
static void SCR_ProjToCanvas(const vec3_t proj, const canvasmap_t *remap,
...
==============
*/
static void SCR_DrawKeyValueOverlay(float x, float y, const char *multistr,
  ...
  // Draw background box
  Draw_FillEx(x - maxkey, y, maxkey + maxval, numlines * CHARSIZE, bgcolor,
              0.75f);
...
void SCR_SetUpToDrawConsole(void) {
  ...
  conspeed =
      (scr_conspeed.value > 0 && !cls.timedemo) ? scr_conspeed.value : 1e6f;

  // make sure the console isn't too slow to come down if we're forcing it open
  // to preview a specific option
...
==================
*/
static qboolean SCR_ExpandVariables(const char *fmt, char *dst,
                                    size_t maxchars) {
  time_t now;
  struct tm *lt;
  char var[256];
...
#define IS_VAR(s) (k == strlen(s) && q_strncasecmp(fmt + i, s, k) == 0)
    ...
    if (IS_VAR("map")) {
      ...
      q_snprintf(var, sizeof(var), "%04d-%02d-%02d", lt->tm_year + 1900,
                 lt->tm_mon + 1, lt->tm_mday);
    else if (IS_VAR("time"))
      q_snprintf(var, sizeof(var), "%02d-%02d-%02d", lt->tm_hour, lt->tm_min,
                 lt->tm_sec);
    else if (IS_VAR("year"))
