...
cvar_t max_edicts = {"max_edicts", "16384",
...
void Host_WriteConfig_f(void) {
  ...
  q_strlcpy(filename, Cmd_Argc() >= 2 ? Cmd_Argv(1) : CONFIG_NAME,
            sizeof(filename));
...
static void AsyncQueue_Push(asyncqueue_t *queue, void (*func)(void *param),
                            void *param) {
  asyncproc_t *proc;
...
static void Host_CheckAutosave(void) {
  ...
  // lower health)
  score *= q_min(100.f, (sv_player->v.health +
    ...
    q_snprintf(title, sizeof(title),
               utf8name[0] ? "%s (%s)  |  skill %d  |  %d/%d kills  |  %d/%d "
                           ...
                           : "%s%s  |  skill %d  |  %d/%d kills  |  %d/%d "
                             "secrets  -  " WINDOW_TITLE_STRING,
               utf8name, current.map, current.stats.skill,
...
==================
*/
static void Host_PrintTimes(const double times[], const char *names[],
                            int count, qboolean showtotal) {
  char line[1024];
  double total = 0.0;
  int i, worst;
