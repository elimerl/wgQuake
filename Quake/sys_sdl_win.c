...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4091) // 'typedef ': ignored on left of 'tagGPFIDL_FLAGS' when no
...
qfileofs_t Sys_ftell(FILE *file) { return _ftelli64(file); }
...
static qboolean Sys_GetRegistryString(HKEY root, const wchar_t *dir,
                                      const wchar_t *keyname, char *out,
                                      size_t maxchars) {
  LSTATUS err;
  HKEY key;
  ...
  *out = 0;
  ...
  if (err != ERROR_SUCCESS || type != REG_SZ ||
      size > sizeof(wpath) - sizeof(wpath[0])) {
    RegCloseKey(key);
    return false;
  }
  ...
  if (WideCharToMultiByte(CP_UTF8, 0, wpath, -1, out, maxchars, NULL, NULL) !=
      0)
    return true;
  *out = 0;
  return false;
...
static qboolean Sys_GetKnownFolder(const KNOWNFOLDERID *base,
                                   const char *subdir, char *path,
                                   size_t pathsize) {
  PWSTR wpath;
  HRESULT hr;
  qboolean ret;
  ...
  ret = WideCharToMultiByte(CP_UTF8, 0, wpath, -1, path, pathsize, NULL,
                            NULL) != 0;
  CoTaskMemFree(wpath);
  CoUninitialize();
...
static qboolean Sys_GetNightdiveDir(char *path, size_t pathsize) {
  return Sys_GetKnownFolder(&FOLDERID_SavedGames, "\\Nightdive Studios\\Quake",
                            path, pathsize);
}

qboolean Sys_GetAltUserPrefDir(qboolean remastered, char *path,
                               size_t pathsize) {
  const char *subdir = remastered ? "\\" ENGINE_USERDIR_WIN "\\rerelease"
...
static void Sys_GetBasedir(char *argv0, char *dst, size_t dstsize) {
  ...
  if (rc == 0)
    Sys_Error("Couldn't determine current directory name length (error %lu)",
              GetLastError());
  if (rc >= countof(wpath))
    Sys_Error("Current directory name too long (%lu)", (DWORD)rc);
  if (!GetCurrentDirectoryW(rc, wpath))
    Sys_Error("Couldn't determine current directory (error %lu)",
              GetLastError());
...
static const char *Sys_GetExeDir(void) {
  ...
  if (!WideCharToMultiByte(CP_UTF8, 0, wpath, -1, exedir, sizeof(exedir), NULL,
                           NULL))
    return NULL;
...
static void Sys_FillFindData(winfindfile_t *find) {
  WideStringToUTF8(find->data.cFileName, find->base.name,
                   countof(find->base.name));
  find->base.attribs = 0;
...
LRESULT CALLBACK KeyFilter(int nCode, WPARAM wParam, LPARAM lParam) {
  if (nCode >= 0 && VID_HasMouseOrInputFocus()) {
    ...
    if (key != -1) {
      ...
      qboolean intercept = force_intercept || key == HK_PRINTSCREEN ||
...
qboolean Sys_IsDebuggerPresent(void) { return IsDebuggerPresent(); }
...
void Sys_ActivateKeyFilter(qboolean active) {
  ...
  if (key_hook) {
    ...
    key_hook =
        SetWindowsHookExW(WH_KEYBOARD_LL, KeyFilter, GetModuleHandleW(NULL), 0);
    if (!key_hook)
...
void Sys_CloseLibrary(void *lib) { FreeLibrary((HMODULE)lib); }
