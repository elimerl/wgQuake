...
void S_StartSound(int entnum, int entchannel, sfx_t *sfx, vec3_t origin,
  ...
  if (cls.demoplayback && cls.demospeed > 0.f && sc->loopstart != -1)
    CL_AddDemoRewindSound(entnum, entchannel, old_sfx, old_origin, old_vol,
                          old_atten);
...
static void S_StopAllSoundsC(void) { S_StopAllSounds(true); }
      ...
      if (ch->sfx && (ch->leftvol || ch->rightvol)) {
        ...
        if (snd_show.value >= 2.f)
          Con_SafePrintf("L:%3i R:%3i | ENT:%5i CH:%3i | %s%s\n", ch->leftvol,
                         ch->rightvol, ch->entnum, ch->entchannel,
                         ch->sfx->name, sc && sc->loopstart >= 0 ? " [L]" : "");
        total++;
      }
    }
