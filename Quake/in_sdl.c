...
cvar_t gyro_mode = {"gyro_mode", "2",
                    CVAR_ARCHIVE}; // 2 = GYRO_BUTTON_DISABLES (see gyromode_t)
...
static void IN_ResetFlickState(void) { memset(&flick, 0, sizeof(flick)); }
...
void IN_DeactivateForConsole(void) { IN_Deactivate(true); }
...
static qboolean IN_UseController(int device_index) {
  ...
  if (SDL_GameControllerHasSensor(joy_active_controller, SDL_SENSOR_GYRO) &&
      !SDL_GameControllerSetSensorEnabled(joy_active_controller,
...
void IN_ShutdownJoystick(void) { SDL_QuitSubSystem(SDL_INIT_GAMECONTROLLER); }

qboolean IN_HasGamepad(void) { return joy_active_controller != NULL; }

const char *IN_GetGamepadName(void) {
  return joy_active_controller ? joy_active_name : NULL;
}

gamepadtype_t IN_GetGamepadType(void) { return joy_active_type; }
...
qboolean IN_HasRumble(void) { return joy_has_rumble; }

void IN_GyroActionDown(void) { gyro_button_pressed = true; }

void IN_GyroActionUp(void) { gyro_button_pressed = false; }
...
================
*/
static void Joy_Flick_f(cvar_t *cvar) { IN_ResetFlickState(); }
...
static void IN_UpdateSDLTextInput(void) {
  // For devices with an on-screen keyboard (e.g. Steam Deck) we only start text
  // input if requested explicitly (TEXTMODE_ON) in order to avoid having the
  // OSK pop up in searchable menus (Options, Maps, or Mods), which use emulated
  // char events instead. For all the other devices we start text input if not
  // disabled explicitly (TEXTMODE_OFF). In other words, TEXTMODE_NOPOPUP is
  // treated as OFF on Steam Deck, ON on desktop.
  qboolean enabled = SDL_HasScreenKeyboardSupport() ? textmode == TEXTMODE_ON
                                                    : textmode != TEXTMODE_OFF;
  if (enabled)
    SDL_StartTextInput();
  else
    SDL_StopTextInput();
...
static int IN_KeyForControllerButton(SDL_GameControllerButton button) {
  ...
  case SDL_CONTROLLER_BUTTON_Y:
    return K_YBUTTON;
  // Note: back and start are always mapped to TAB/ESC, the player cannot rebind
  // them
...
================
*/
static void IN_JoyKeyEvent(qboolean wasdown, qboolean isdown, int key,
                           double *timer) {
...
void IN_Commands(void) {
  ...
  if (key_dest != key_game) {
    ...
    IN_JoyKeyEvent(joy_axisstate.axisvalue[xaxis] < -stickthreshold,
                   newaxisstate.axisvalue[xaxis] < -stickthreshold, K_LEFTARROW,
                   &joy_emulatedkeytimer[0]);
    IN_JoyKeyEvent(joy_axisstate.axisvalue[xaxis] > stickthreshold,
                   newaxisstate.axisvalue[xaxis] > stickthreshold, K_RIGHTARROW,
                   &joy_emulatedkeytimer[1]);
    IN_JoyKeyEvent(joy_axisstate.axisvalue[yaxis] < -stickthreshold,
                   newaxisstate.axisvalue[yaxis] < -stickthreshold, K_UPARROW,
                   &joy_emulatedkeytimer[2]);
    IN_JoyKeyEvent(joy_axisstate.axisvalue[yaxis] > stickthreshold,
                   newaxisstate.axisvalue[yaxis] > stickthreshold, K_DOWNARROW,
  ...
  if (key_dest == key_console) {
    ...
    deadzone = IN_ApplyDeadzone(raw, joy_deadzone_look.value,
  ...
  if (joy_has_rumble && !IN_IsCalibratingGyro() && joy_rumble.value > 0.f) {
    ...
    hifreq *= hifreq;
    SDL_GameControllerRumble(joy_active_controller, lofreq * strength,
                             hifreq * strength, 100);
  }
#endif // SDL_VERSION_ATLEAST (2, 0, 9)
}
...
static float IN_RecenterEasing(float frac) { return frac * frac; }
...
keydevice_t IN_GetLastActiveDeviceType(void) { return lastactivetype; }
...
static qboolean IN_UpdateGyroCalibration(const float newsample[3]) {
  ...
  if (!updates_countdown) {
    ...
    Con_Printf("Calibration results:\n X=%f Y=%f Z=%f\n",
               gyro_calibration_x.value, gyro_calibration_y.value,
...
qboolean IN_HasGyro(void) { return gyro_present; }
...
float IN_GetRawMoveMagnitude(void) {
  ...
  axis.x = joy_axisstate
               .axisvalue[joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_RIGHTX
                                                 : SDL_CONTROLLER_AXIS_LEFTX];
  axis.y = joy_axisstate
               .axisvalue[joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_RIGHTY
                                                 : SDL_CONTROLLER_AXIS_LEFTY];
...
qboolean IN_IsCalibratingGyro(void) { return updates_countdown != 0; }
...
void IN_SendKeyEvents(void) {
  ...
  while (SDL_PollEvent(&event)) {
    ...
    case SDL_KEYUP:
      ...
      // also pass along the underlying keycode using the proper current layout
      // for Y/N prompts.
