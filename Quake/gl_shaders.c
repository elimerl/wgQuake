...
static void GL_InitError(const char *message, ...) {
  ...
  Sys_Error(fmt, buf, (int)sizeof(void *) * 8, gl_version, gl_renderer,
            gl_vendor);
}
...
=============
*/
static qboolean AppendString(char **dst, const char *dstend, const char *str,
                             int len) {
...
=============
*/
static GLuint GL_CreateShader(GLenum type, const char *source,
                              const char *extradefs, const char *name) {
  ...
  q_snprintf(header, sizeof(header),
             ...
             "#define BINDLESS %d\n"
             "#define REVERSED_Z %d\n",
             gl_bindless_able, gl_clipcontrol_able);
...
=============
*/
static GLuint GL_CreateProgramFromShaders(const GLuint *shaders, int numshaders,
                                          const char *name) {
  GLuint program;
  GLint status;
...
====================
*/
static GLuint GL_CreateProgramFromSources(int count, const GLchar **sources,
                                          const GLenum *types, const char *name,
                                          va_list argptr) {
  char macros[1024];
  char eval[256];
  char *pipe;
...
====================
*/
static FUNC_PRINTF(3, 4) GLuint
    GL_CreateProgram(const GLchar *vertSource, const GLchar *fragSource,
                     const char *name, ...) {
...
void GL_CreateShaders(void) {
  ...
  glprogs.viewblend = GL_CreateProgram(viewblend_vertex_shader,
                                       viewblend_fragment_shader, "viewblend");
  for (warp = 0; warp < 2; warp++)
    glprogs.warpscale[warp] =
        GL_CreateProgram(warpscale_vertex_shader, warpscale_fragment_shader,
                         "view warp/scale|WARP %d", warp);
  for (palettize = 0; palettize < 3; palettize++)
    glprogs.postprocess[palettize] =
        GL_CreateProgram(postprocess_vertex_shader, postprocess_fragment_shader,
                         "postprocess|PALETTIZE %d", palettize);

  for (mode = 0; mode < 2; mode++)
    glprogs.oit_resolve[mode] =
        GL_CreateProgram(oit_resove_vertex_shader, oit_resove_fragment_shader,
                         "oit resolve|MSAA %d", mode);

  for (oit = 0; oit < 2; oit++)
    for (dither = 0; dither < 3; dither++)
      for (mode = 0; mode < 3; mode++)
        glprogs.world[oit][dither][mode] = GL_CreateProgram(
            world_vertex_shader, world_fragment_shader,
            "world|OIT %d; DITHER %d; MODE %d", oit, dither, mode);

  for (dither = 0; dither < 2; dither++) {
    for (oit = 0; oit < 2; oit++) {
      glprogs.water[oit][dither] =
          GL_CreateProgram(water_vertex_shader, water_fragment_shader,
                           "water|OIT %d; DITHER %d", oit, dither);
      glprogs.particles[oit][dither] =
          GL_CreateProgram(particles_vertex_shader, particles_fragment_shader,
                           "particles|OIT %d; DITHER %d", oit, dither);
    }
    for (mode = 0; mode < 2; mode++)
      glprogs.skycubemap[mode][dither] = GL_CreateProgram(
          sky_cubemap_vertex_shader, sky_cubemap_fragment_shader,
          "sky cubemap|ANIM %d; DITHER %d", mode, dither);
    glprogs.skylayers[dither] =
        GL_CreateProgram(sky_layers_vertex_shader, sky_layers_fragment_shader,
                         "sky layers|DITHER %d", dither);
    glprogs.skyboxside[dither] =
        GL_CreateProgram(sky_boxside_vertex_shader, sky_boxside_fragment_shader,
                         "skybox side|DITHER %d", dither);
    glprogs.sprites[dither] =
        GL_CreateProgram(sprites_vertex_shader, sprites_fragment_shader,
                         "sprites|DITHER %d", dither);
  }
  glprogs.skystencil =
        ...
        for (md5 = 0; md5 < 2; md5++)
          glprogs.alias[oit][mode][alphatest][md5] =
              GL_CreateProgram(alias_vertex_shader, alias_fragment_shader,
                               ...
                               oit, mode, alphatest, md5);

  glprogs.debug3d = GL_CreateProgram(debug3d_vertex_shader,
                                     debug3d_fragment_shader, "debug3d");
