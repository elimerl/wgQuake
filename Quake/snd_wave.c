...
=================
*/
static qboolean WAV_ReadRIFFHeader(const char *name, FILE *file,
                                   snd_info_t *info) {
  char dump[16];
  int wav_format;
  int fmtlen = 0;
...
int S_WAV_CodecReadStream(snd_stream_t *stream, int bytes, void *buffer) {
  ...
  if (fread(buffer, 1, bytes, stream->fh.file) != bytes)
    Sys_Error("S_WAV_CodecReadStream: read error on %d bytes (%s)", bytes,
              stream->name);
...
static qboolean S_WAV_CodecInitialize(void) { return true; }
