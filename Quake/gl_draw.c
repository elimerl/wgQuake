...
void Scrap_Upload(void) {
  scrap_texture = TexMgr_LoadImage(
      NULL, "scrap", SCRAP_ATLAS_WIDTH, SCRAP_ATLAS_HEIGHT, SRC_INDEXED,
      scrap_texels, "", (src_offset_t)scrap_texels,
...
================
*/
void Draw_FillClampTexels(const byte *src, int w, int h, int srcpitch,
                          byte *dst, int dstpitch) {
  int i;
...
qpic_t *Draw_PicFromWad2(const char *name, unsigned int texflags) {
  ...
  if ((size_t)info->size > sizeof(int) * 2 + p->width * p->height)
    Con_DPrintf("Draw_PicFromWad: pic \"%s\" over-sized (%u*%u requires only "
                "%u bytes)\n",
                name, p->width, p->height, 8 + p->width * p->height);
  ...
  if (Scrap_Compatible(texflags) &&
    ...
    gl.gltexture = TexMgr_LoadImage(NULL, texturename, p->width, p->height,
                                    SRC_INDEXED, p->data, WADFILENAME, offset,
...
qpic_t *Draw_CachePic(const char *path) {
  qpic_t *pic = Draw_TryCachePic(path, TEXPREF_ALPHA | TEXPREF_PAD |
                                           TEXPREF_NOPICMIP | TEXPREF_CLAMP);
  if (!pic)
...
void Draw_LoadPics(void) {
  ...
  char_texture = TexMgr_LoadImage(
      NULL, WADFILENAME ":conchars", 16 * 10, 16 * 10, SRC_INDEXED,
      char_texture_data, "", (src_offset_t)char_texture_data,
...
static void Draw_CreateWinQuakeMenuBgTex(void) {
  ...
  winquakemenubg = TexMgr_LoadImage(
      NULL, "winquakemenubg", 4, 2, SRC_RGBA, (byte *)winquakemenubg_pixels, "",
...
void Draw_Flush(void) {
  ...
  GL_Upload(GL_ARRAY_BUFFER, batchverts,
            sizeof(batchverts[0]) * 4 * numbatchquads, &buf, &ofs);
  GL_BindBuffer(GL_ARRAY_BUFFER, buf);
  GL_VertexAttribPointerFunc(0, 2, GL_FLOAT, GL_FALSE, sizeof(batchverts[0]),
                             ofs + offsetof(guivertex_t, pos));
  GL_VertexAttribPointerFunc(1, 2, GL_FLOAT, GL_FALSE, sizeof(batchverts[0]),
                             ofs + offsetof(guivertex_t, uv));
  GL_VertexAttribPointerFunc(2, 4, GL_UNSIGNED_BYTE, GL_TRUE,
                             sizeof(batchverts[0]),
                             ofs + offsetof(guivertex_t, color));

  GL_Upload(GL_ELEMENT_ARRAY_BUFFER, batchindices,
            sizeof(batchindices[0]) * 6 * numbatchquads, &buf, &ofs);
...
================
*/
void Draw_String(int x, int y, const char *str) { Draw_StringEx(x, y, 8, str); }
...
void Draw_SubPic(float x, float y, float w, float h, qpic_t *pic, float s1,
                 float t1, float s2, float t2, const float *rgb, float alpha) {
  glpic_t *gl;
  guivertex_t *verts;
  ...
  verts = Draw_AllocQuad();
  Draw_SetVertex(verts++, x, y, LERP(gl->sl, gl->sh, s1),
                 LERP(gl->tl, gl->th, t1));
  Draw_SetVertex(verts++, x + w, y, LERP(gl->sl, gl->sh, s2),
                 LERP(gl->tl, gl->th, t1));
  Draw_SetVertex(verts++, x + w, y + h, LERP(gl->sl, gl->sh, s2),
                 LERP(gl->tl, gl->th, t2));
  Draw_SetVertex(verts++, x, y + h, LERP(gl->sl, gl->sh, s1),
                 LERP(gl->tl, gl->th, t2));
...
void Draw_TileClear(int x, int y, int w, int h) {
  ...
  uvscale = 1.f / 64.f /
            CLAMP(1.0f, scr_sbarscale.value,
                  (float)vid.guiwidth / 320.0f); // use sbar scale
  uvbias = -vid.guiheight * uvscale;             // bottom-aligned tiles

  verts = Draw_AllocQuad();
  Draw_SetVertex(verts++, x * scalex, y * scaley, x * scalex * uvscale,
                 y * scaley * uvscale + uvbias);
  Draw_SetVertex(verts++, (x + w) * scalex, y * scaley,
                 (x + w) * scalex * uvscale, y * scaley * uvscale + uvbias);
  Draw_SetVertex(verts++, (x + w) * scalex, (y + h) * scaley,
                 (x + w) * scalex * uvscale,
                 (y + h) * scaley * uvscale + uvbias);
  Draw_SetVertex(verts++, x * scalex, (y + h) * scaley, x * scalex * uvscale,
                 (y + h) * scaley * uvscale + uvbias);
...
=============
*/
void Draw_FillEx(float x, float y, float w, float h, const float *rgb,
                 float alpha) {
  guivertex_t *verts;
...
void Draw_FadeScreen(float alpha) {
  Draw_PartialFadeScreen(glcanvas.left, glcanvas.right, glcanvas.top,
                         glcanvas.bottom, alpha);
}
...
================
*/
static void Draw_Transform2(float width, float height, float scalex,
                            float scaley, float alignx, float aligny,
                            drawtransform_t *out) {
...
================
*/
static void Draw_Transform(float width, float height, float scale, float alignx,
                           float aligny, drawtransform_t *out) {
...
void Draw_GetCanvasTransform(canvastype type, drawtransform_t *transform) {
  ...
  switch (type) {
  case CANVAS_DEFAULT:
    Draw_Transform(vid.guiwidth, vid.guiheight, 1.f, CANVAS_ALIGN_CENTERX,
                   CANVAS_ALIGN_CENTERY, transform);
    break;
  case CANVAS_CONSOLE:
    s = (float)vid.guiwidth / vid.conwidth; // use console scale
    s2 = (float)vid.guiheight / vid.conheight;
    Draw_Transform2(vid.conwidth, vid.conheight, s, s2, CANVAS_ALIGN_CENTERX,
                    CANVAS_ALIGN_CENTERY, transform);
  ...
  case CANVAS_MENU:
    s = q_min((float)vid.guiwidth / 320.0f, (float)vid.guiheight / 200.0f);
    s = CLAMP(1.0f, scr_menuscale.value, s);
    Draw_Transform(320, 200, s, CANVAS_ALIGN_CENTERX, CANVAS_ALIGN_CENTERY,
                   transform);
    break;
  case CANVAS_CSQC:
    s = CLAMP(1.0f, scr_sbarscale.value, vid.guiwidth / 320.0f);
    Draw_Transform(vid.guiwidth / s, vid.guiheight / s, s, CANVAS_ALIGN_CENTERX,
                   CANVAS_ALIGN_CENTERY, transform);
    break;
  case CANVAS_SBAR:
    ...
    if (cl.gametype == GAME_DEATHMATCH &&
      ...
      Draw_Transform(320, 48, s, CANVAS_ALIGN_LEFT, CANVAS_ALIGN_BOTTOM,
                     transform);
    else
      Draw_Transform(320, 48, s, CANVAS_ALIGN_CENTERX, CANVAS_ALIGN_BOTTOM,
                     transform);
    break;
  case CANVAS_SBAR_QW_INV:
    s = CLAMP(1.0f, scr_sbarscale.value, (float)vid.guiheight / 240.0f);
    Draw_Transform(48, 48, s, CANVAS_ALIGN_RIGHT, CANVAS_ALIGN_BOTTOM,
                   transform);
    break;
  case CANVAS_SBAR2:
  ...
  case CANVAS_CROSSHAIR: // 0,0 is center of viewport
    ...
    Draw_Transform(vid.guiwidth / s / 2, vid.guiheight / s / 2, s,
                   CANVAS_ALIGN_LEFT, CANVAS_ALIGN_BOTTOM, transform);
  ...
  case CANVAS_BOTTOMLEFT:                   // used by devstats
    s = (float)vid.guiwidth / vid.conwidth; // use console scale
    Draw_Transform(320, 200, s, CANVAS_ALIGN_LEFT, CANVAS_ALIGN_BOTTOM,
                   transform);
    break;
  case CANVAS_BOTTOMRIGHT:                  // used by fps/clock
    s = (float)vid.guiwidth / vid.conwidth; // use console scale
    Draw_Transform(320, 200, s, CANVAS_ALIGN_RIGHT, CANVAS_ALIGN_BOTTOM,
                   transform);
    break;
  case CANVAS_TOPRIGHT:                     // used by disc
    s = (float)vid.guiwidth / vid.conwidth; // use console scale
    Draw_Transform(320, 200, s, CANVAS_ALIGN_RIGHT, CANVAS_ALIGN_TOP,
                   transform);
    break;
  default:
...
================
*/
void Draw_GetTransformBounds(const drawtransform_t *transform, float *left,
...
void GL_SetCanvas(canvastype newcanvas) {
  ...
  Draw_GetTransformBounds(&glcanvas.transform, &glcanvas.left, &glcanvas.top,
                          &glcanvas.right, &glcanvas.bottom);
}
