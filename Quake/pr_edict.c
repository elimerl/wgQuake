...
static const char *PR_ValueString(int type, eval_t *val) {
  ...
  case ev_float:
    ...
    break;
  case ev_vector:
    q_snprintf(fmt, sizeof(fmt), "'%s %s %s'", PR_FloatFormat(val->vector[0]),
               PR_FloatFormat(val->vector[1]), PR_FloatFormat(val->vector[2]));
    q_snprintf(line, sizeof(line), fmt, val->vector[0], val->vector[1],
               val->vector[2]);
    break;
  case ev_pointer:
...
============
*/
static const char *PR_UglySaveValueString(savedata_t *save, int type,
                                          eval_t *val) {
  static char line[1024];
  ddef_t *def;
  dfunction_t *f;
...
static void ED_PrintEdict_Completion_f(const char *partial) {
  ...
  for (i = 0; i < (int)VEC_SIZE(bbox_linked); i++) {
    ...
    Con_AddToTabList(va("%d", NUM_FOR_EDICT(ed)), partial,
...
static qboolean ED_ParseEpair(void *base, ddef_t *key, const char *s,
  ...
  case ev_field:
    def = ED_FindField(s);
    if (!def) {
      // johnfitz -- HACK -- suppress error becuase fog/sky fields might not be
      // mentioned in defs.qc
...
void PR_UnzoneAll(void) { // called to clean up all zoned strings.
...
static func_t PR_FindExtFunction(
    const char *entryname) { // depends on 0 being an invalid function,
  ...
  for (i = 0; i < (unsigned int)qcvm->progs->numglobaldefs; i++) {
    ...
    if (!strncmp(n, "autocvar_", 9)) {
      ...
      cvar_t *var = Cvar_Create(
          n + 9, PR_UglyValueString(
                     qcvm->globaldefs[i].type,
                     (eval_t *)(qcvm->globals + qcvm->globaldefs[i].ofs)));
      numautocvars++;
      if (!var)
        continue; // name conflicts with a command?

      if (!ED_ParseEpair((void *)qcvm->globals, &qcvm->globaldefs[i],
                         var->string, true))
        Con_Warning("EXT: Unable to configure %s\n", n);
...
static void PR_MergeEngineFieldDefs(void) {
  struct {
    ...
    int newidx;
      ...
      // these later. this is useful for fields that should be defined for
      // mappers which are not defined by the mod. future note: mutators will
      // need to edit the mutator's globaldefs table too. remember to handle
      // vectors and their 3 globals too.
  ...
  if (maxdefs !=
    ...
    for (j = 0; j < countof(extrafields); j++) {
      if (extrafields[j].newidx >= qcvm->progs->entityfields &&
          extrafields[j].newidx < maxofs) { // looks like its new. make sure
                                            // ED_FindField can find it.
...
static void PR_FindEntityFields(void) {
  ...
  qcvm->entityfieldofs = (int *)Hunk_AllocName(
      qcvm->numentityfields * sizeof(int), "entityfieldofs");
...
qboolean PR_LoadProgs(const char *filename, qboolean fatal) {
  ...
  if (qcvm->progs->version != PROG_VERSION) {
    if (fatal)
      Host_Error("%s has wrong version number (%i should be %i)", filename,
                 qcvm->progs->version, PROG_VERSION);
    else {
...
void SaveData_Fill(savedata_t *save) {
  ...
  memcpy(save->globals, qcvm->globals,
