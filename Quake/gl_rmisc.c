...
void R_TranslateNewPlayerSkin(int playernum) {
  ...
  playertextures[playernum] = TexMgr_LoadImage(
      e->model, name, paliashdr->skinwidth, paliashdr->skinheight, SRC_INDEXED,
      pixels, paliashdr->gltextures[skinnum][0]->source_file,
  ...
  for (i = 0; i < countof(frameres); i++) {
    ...
    if (bits & FRAMERES_HOST_BUFFER_BIT) {
      ...
      if (gl_buffer_storage_able) {
        GL_BufferStorageFunc(GL_ARRAY_BUFFER, frameres_host_buffer_size, NULL,
                             flags);
        ...
        GL_BufferDataFunc(GL_ARRAY_BUFFER, frameres_host_buffer_size, NULL,
                          GL_STREAM_DRAW);
      }
    }
    ...
    if (bits & FRAMERES_DEVICE_BUFFER_BIT) {
      ...
      GL_BufferDataFunc(GL_SHADER_STORAGE_BUFFER, frameres_device_buffer_size,
                        NULL, GL_STREAM_DRAW);
    }
  }
...
void GL_AcquireFrameResources(void) {
  ...
  if (frame->fence) {
    ...
    GLenum result = GL_ClientWaitSyncFunc(frame->fence,
