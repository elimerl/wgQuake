...
===================
*/
static int Hunk_Size(void) { return LASTSEG->base + LASTSEG->size; }
  ...
  for (i = 0; i < hunk_numsegments; i++) {
    ...
    if (seg->base < hunk_low_used) {
      ...
      for (ofs = 0; ofs < seg->used;) {
        ...
        // print the total
        //
        if (!next || strncmp(Hunk_GetName(h), Hunk_GetName(next),
                             HUNKNAME_LEN - 1) != 0) {
          if (!all)
...
===================
*/
void *Hunk_Alloc(int size) { return Hunk_AllocName(size, NULL); }
...
===================
*/
void *Hunk_AllocNoFill(int size) { return Hunk_AllocNameNoFill(size, NULL); }
