...
static void Mod_LoadTextures(lump_t *l) {
  ...
  for (i = 0; i < nummiptex; i++) {
    ...
    {
      if (!q_strncasecmp(tx->name, "sky",
                         3)) // sky texture //also note -- was Q_strncmp,
                             // changed to match qbsp
      {
        if (loadmodel->bspversion == BSPVERSION_QUAKE64)
      ...
      } else if (tx->name[0] == '*') // warping texture
      {
        ...
        if (data) // load external image
        {
          ...
          tx->gltexture =
              TexMgr_LoadImage(loadmodel, texturename, fwidth, fheight,
        ...
        tx->warpimage = TexMgr_LoadImage(
            ...
            SRC_RGBA, hunk_base, "", (src_offset_t)hunk_base, flags);
        tx->update_warp = true;
      } else // regular texture
      {
        ...
        if (data) // load external image
        {
          ...
          tx->gltexture =
              TexMgr_LoadImage(loadmodel, filename, fwidth, fheight, SRC_RGBA,
                               data, filename, 0, TEXPREF_MIPMAP | extraflags);
          ...
          if (data)
            tx->fullbright = TexMgr_LoadImage(
                loadmodel, filename2, fwidth, fheight, SRC_RGBA, data,
                filename2, 0, TEXPREF_MIPMAP | extraflags);
        } else // use the texture from the bsp file
        {
          q_snprintf(texturename, sizeof(texturename), "%s:%s", loadmodel->name,
                     tx->name);
...
static void Mod_LoadLighting(lump_t *l) {
  ...
  if (loadmodel->bspversion == BSPVERSION_QUAKE64) {
    ...
    loadmodel->lightdata =
        (byte *)Hunk_AllocName((l->filelen / 2) * 3, litfilename);
