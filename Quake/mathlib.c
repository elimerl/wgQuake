...
float Log2f(float val) { return log(val) * 1.44269504; }

float Exp2f(float val) { return exp(val * 0.693147181); }
...
qboolean RayVsBox(const vec3_t org, const vec3_t rcpdelta, const vec3_t mins,
                  const vec3_t maxs, float *frac) {
  int i;
  float enter, exit;
