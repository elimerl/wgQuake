...
static void PF_precache_file(void) { // precache_file is only used to copy files
                                     // with qcc, it does nothing
...
static void PF_traceon(void) { qcvm->trace = true; }

static void PF_traceoff(void) { qcvm->trace = false; }
...
static void PF_floor(void) { G_FLOAT(OFS_RETURN) = floor(G_FLOAT(OFS_PARM0)); }

static void PF_ceil(void) { G_FLOAT(OFS_RETURN) = ceil(G_FLOAT(OFS_PARM0)); }
...
==============
*/
static void PF_finalefinished(void) { G_FLOAT(OFS_RETURN) = 0; }
static void PF_CheckPlayerEXFlags(void) { G_FLOAT(OFS_RETURN) = 0; }
...
void PF_Fixme(void) { PR_RunError("unimplemented builtin"); }
...
void PR_Markup_Begin(struct markup_s *mu, const char *text, float *rgb,
                     float alpha) {
  if (*text == '\1' || *text == '\2') {
    mu->mask = 128;
    text++;
...
int PR_Markup_Parse(struct markup_s *mu) {
  static const vec4_t q3rgb[10] = {
      {0.00, 0.00, 0.00, 1.0}, {1.00, 0.33, 0.33, 1.0}, {0.00, 1.00, 0.33, 1.0},
      {1.00, 1.00, 0.33, 1.0}, {0.33, 0.33, 1.00, 1.0}, {0.33, 1.00, 1.00, 1.0},
      {1.00, 0.33, 1.00, 1.0}, {1.00, 1.00, 1.00, 1.0}, {1.00, 1.00, 1.00, 0.5},
      {0.50, 0.50, 0.50, 1.0}};
  unsigned int c;
  const float *f;
  while ((c = *mu->txt)) {
    if (c == '^' &&
      ...
      case '{': // full unicode codepoint, for chars up to 0x10ffff
        ...
        // well as translate cyrilic somehow, but not really necessary.
          ...
          c = '?'; // otherwise its some unicode char that we don't know how to
                   // handle.
        return c;
      }
    }
...
void PF_cl_playerkey_internal(int player, const char *key, qboolean retfloat) {
  ...
  if (player < 0 || player >= MAX_SCOREBOARD)
    ...
    q_snprintf(buf, sizeof(buf), "%i", cl.scores[player].colors & 0x0f);
               ...
               key,
               "team")) // quakeworld uses team infokeys to decide teams
...
static qpic_t *DrawQC_CachePic(
    const char *picname,
    unsigned int flags) { // okay, so this is silly. we've ended up with 3
                          // different cache levels. qcpics, pics, and images.
  size_t i;
...
static void PF_cl_drawresetclip(void) { Draw_ResetClipping(); }
...
// maths stuff
static void PF_Sin(void) { G_FLOAT(OFS_RETURN) = sin(G_FLOAT(OFS_PARM0)); }
static void PF_asin(void) { G_FLOAT(OFS_RETURN) = asin(G_FLOAT(OFS_PARM0)); }
static void PF_Cos(void) { G_FLOAT(OFS_RETURN) = cos(G_FLOAT(OFS_PARM0)); }
static void PF_acos(void) { G_FLOAT(OFS_RETURN) = acos(G_FLOAT(OFS_PARM0)); }
static void PF_tan(void) { G_FLOAT(OFS_RETURN) = tan(G_FLOAT(OFS_PARM0)); }
static void PF_atan(void) { G_FLOAT(OFS_RETURN) = atan(G_FLOAT(OFS_PARM0)); }
static void PF_atan2(void) {
  G_FLOAT(OFS_RETURN) = atan2(G_FLOAT(OFS_PARM0), G_FLOAT(OFS_PARM1));
}
static void PF_Sqrt(void) { G_FLOAT(OFS_RETURN) = sqrt(G_FLOAT(OFS_PARM0)); }
...
// string stuff
static void PF_strlen(void) { // FIXME: doesn't try to handle utf-8
...
static void PF_sprintf_internal(const char *s, int firstarg, char *outbuf,
                                int outbuflen) {
  const char *s0;
        ...
        *f++ = 0;
        ...
        case 'i':
          if (precision < 0) // not set
            ...
            q_snprintf(o, end - o, formatbuf, width, precision,
                       (isfloat ? (int)GETARG_FLOAT(thisarg)
                                : (int)GETARG_INT(thisarg)));
          o += strlen(o);
          break;
        ...
        case 'V':
          ...
          if (precision < 0) // not set
            q_snprintf(o, end - o,
                       va("%s %s %s", /* NESTED SPRINTF IS NESTED */ formatbuf,
                          formatbuf, formatbuf),
                       width,
        ...
        case 'c':
          ...
          /*							else
                                                                  {
                                                                          unsigned
             int c = (isfloat ? (unsigned int) GETARG_FLOAT(thisarg) : (unsigned
             int) GETARG_INT(thisarg)); char charbuf16[16]; const char *buf =
             u8_encodech(c, NULL, charbuf16); if(!buf) buf = ""; if(precision <
             0) // not set precision = end - o - 1; o += u8_strpad(o, end - o,
             buf, (flags & PRINTF_LEFT) != 0, width, precision);
                                                                  }
          */
          break;
        case 'S': { // tokenizable string
          ...
          // try and escape it... hopefully it won't get truncated by precision
          // limits...
...
static void PF_ArgC(void) { G_FLOAT(OFS_RETURN) = qctoken_count; }
...
static void PF_stof(void) { G_FLOAT(OFS_RETURN) = atof(G_STRING(OFS_PARM0)); }
...
static void PF_etos(void) { // yes, this is lame
  char *result = PR_GetTempString();
  q_snprintf(result, STRINGTEMP_LENGTH, "entity %i", G_EDICTNUM(OFS_PARM0));
  G_INT(OFS_RETURN) = PR_SetEngineString(result);
}
static void PF_ftoi(void) { G_INT(OFS_RETURN) = G_FLOAT(OFS_PARM0); }
static void PF_itof(void) { G_FLOAT(OFS_RETURN) = G_INT(OFS_PARM0); }
...
builtindef_t pr_builtindefs[] = {
    {"makevectors", PF_SSQC(PF_makevectors),
     1}, // void(entity e) makevectors		= #1
    {"setorigin", PF_SSQC(PF_setorigin),
     2}, // void(entity e, vector o) setorigin	= #2
    {"setmodel", PF_SSQC(PF_setmodel),
     3}, // void(entity e, string m) setmodel	= #3
    {"setsize", PF_SSQC(PF_setsize),
    ...
    {"sound", PF_SSQC(PF_sound),
     8}, // void(entity e, float chan, string samp) sound	= #8
    {"normalize", PF_BOTH(PF_normalize),
    ...
    {"eprint", PF_SSQC(PF_eprint),
     31}, // void(entity e) debug print an entire entity
    {"walkmove", PF_SSQC(PF_walkmove),
    ...
    {"precache_sound2", PF_SSQC(PF_precache_sound),
    ...
    // 2021 re-release
    {"finaleFinished", PF_SSQC(PF_finalefinished),
     79}, // float() finaleFinished = #79
    {"localsound", PF_SSQC(PF_localsound),
    ...
    {"substring", PF_BOTH(PF_substring), 116,
    ...
    {"getplayerkeyvalue", PF_CSQC(PF_cl_playerkey_s),
