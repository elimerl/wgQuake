...
================
*/
void GL_MakeAliasModelDisplayLists(qmodel_t *aliasmodel,
...
void GLMesh_LoadVertexBuffer(qmodel_t *m, aliashdr_t *mainhdr) {
  ...
  for (hdr = mainhdr, numverts = 0, numindexes = 0;;) {
    ...
    numindexes += hdr->numindexes;
    memcpy(ebodata + hdr->eboofs, (short *)((byte *)hdr + hdr->indexes),
    ...
    case PV_QUAKE1:
                ...
                // numposes in QuakeSpasm
      {
        ...
        for (v = 0; v < hdr->numverts_vbo; v++) {
          ...
          // an unsigned char. this introduces some error (less than 0.004), but
          // the normals were very coarse to begin with
          xyz[v].normal[0] =
    ...
    case PV_IQM:
      ...
      memcpy(vbodata + hdr->vboposeofs, (byte *)hdr + hdr->boneposedata,
  ...
  m->meshindexesvbo = GL_CreateBuffer(
      GL_ELEMENT_ARRAY_BUFFER, GL_STATIC_DRAW, va("%s indices", m->name),
      numindexes * sizeof(unsigned short), ebodata);

  // upload vertexes buffer
  GL_DeleteBuffer(m->meshvbo);
  m->meshvbo =
      GL_CreateBuffer(GL_ARRAY_BUFFER, GL_STATIC_DRAW,
                      va("%s vertices", m->name), totalvbosize, vbodata);
